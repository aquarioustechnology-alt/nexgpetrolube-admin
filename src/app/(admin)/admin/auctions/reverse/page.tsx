...
const auctionData = [
  {
    ...
    status: "live",
    bids: [
      { bidder: "ABC Petrochemicals", amount: 4200, time: "2024-01-15T16:30:00Z", qty: "200L" },
      { bidder: "XYZ Lubricants", amount: 4250, time: "2024-01-15T16:25:00Z", qty: "150L" },
      { bidder: "Oil Solutions Ltd", amount: 4300, time: "2024-01-15T16:20:00Z", qty: "300L" }
    ]
  },
  {
    id: "TRD002",
    ...
    status: "completed",
    bids: [
      { bidder: "Transport Solutions", amount: 3800, time: "2024-01-15T16:45:00Z", qty: "100L" },
      { bidder: "Heavy Machinery Co", amount: 3750, time: "2024-01-15T16:40:00Z", qty: "200L" },
      { bidder: "Industrial Corp", amount: 3700, time: "2024-01-15T16:35:00Z", qty: "200L" }
    ]
  },
  {
    id: "REV003",
...
function AuctionDetailSheet({ auction, onAction }: { auction: any, onAction: (action: string, id: string) => void }) {
  return (
    <SheetContent className="w-[600px] sm:w-[800px] overflow-y-auto">
      ...
      <div className="space-y-6 mt-6">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <h4 className="font-semibold text-sm text-muted-foreground mb-2">AUCTION DETAILS</h4>
            <div className="space-y-2">
              ...
              <div className="text-sm text-muted-foreground">
                ...
                Type: {auction.type === 'reverse' ? 'Reverse Auction' : 'Traditional Auction'}
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <div className="text-sm text-muted-foreground">
                {auction.status === 'live' && `Ends in: ${getTimeRemaining(auction.endTime)}`}
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <div className="text-sm text-muted-foreground">Filled Quantity</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-lg font-semibold">
                    Best {auction.type === 'reverse' ? 'Bid' : 'Offer'}: ₹{auction.currentBest.toLocaleString()}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    {auction.type === 'reverse' ? 'Lowest price wins' : 'Highest price wins'}
                  </div>
                </div>
        ...
        <div>
          <h4 className="font-semibold text-sm text-muted-foreground mb-3">LIVE BIDDING TABLE</h4>
              ...
              <TableBody>
                {auction.bids.map((bid: any, index: number) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{new Date(bid.time).toLocaleTimeString()}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
...
export default function ReverseAuctionsPage() {
  ...
  const reverseAuctions = auctionData.filter(auction => auction.type === 'reverse')
  
  const filteredAuctions = reverseAuctions.filter(auction =>
    auction.product.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (auction.buyer && auction.buyer.toLowerCase().includes(searchTerm.toLowerCase()))
  )
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Monitor buyer-led reverse auctions where sellers compete with lowest prices
        </p>
      </div>
                ...
                <TableRow key={auction.id}>
                  ...
                  <TableCell>
                    <div className="text-sm">
                      ...
                      <div className="text-muted-foreground">Filled: {auction.filledQty}</div>
                    </div>
                  </TableCell>
                  <TableCell>
                    {auction.currentBest > 0 ? (
                    ...
                    <div className="flex gap-2">
                      <Sheet>
                        <SheetTrigger asChild>
                          <Button size="sm" variant="outline" onClick={() => setSelectedAuction(auction)}>
                      ...
                      {auction.status === 'live' && (
                        <Button size="sm" variant="outline" onClick={() => handleAuctionAction('pause', auction.id)}>
                      ...
                      {auction.status === 'paused' && (
                        <Button size="sm" onClick={() => handleAuctionAction('resume', auction.id)}>
