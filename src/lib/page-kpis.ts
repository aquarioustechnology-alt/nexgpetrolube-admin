...
export const KPI = {
  users(data: any[]) {
    ...
    return [
      ...
      },
      {
        label: 'Pending KYC',
        value: pending,
        icon: React.createElement(Hourglass, { className: 'h-4 w-4' }),
  ...
  listings(data: any[]) {
    ...
    const approved =
      data?.filter((x: any) => x.status === 'Approved').length ?? 0
    ...
    return [
      ...
      },
      {
        label: 'Bidding Type',
        value: bidding,
        icon: React.createElement(Gavel, { className: 'h-4 w-4' }),
        hint: undefined,
      },
    ]
  },
  requirements(data: any[]) {
    ...
    return [
      ...
      },
      {
        label: 'Reverse Mode',
        value: reverse,
        icon: React.createElement(Repeat2, { className: 'h-4 w-4' }),
        hint: undefined,
      },
      {
        label: 'Regulated Requests',
        value: regulated,
  ...
  auctionsReverse(data: any[]) {
    const live =
      data?.filter((x: any) => x.type === 'Reverse' && x.status === 'Live')
        .length ?? 0
    const bidders = (data || []).reduce(
      (s: number, x: any) => s + (x.bidders || 0),
      0
    )
    const bids = (data || []).reduce(
      (s: number, x: any) => s + (x.bidsToday || 0),
      0
    )
    const premium =
      (data?.length
        ? data.reduce((s: number, x: any) => s + (x.avgPremium || 0), 0) /
          data.length
        : 0
      ).toFixed(1) + '%'
    return [
      ...
      },
      {
        label: 'Bids Today',
        value: bids,
        icon: React.createElement(History, { className: 'h-4 w-4' }),
        hint: 'Sum of live rooms',
      },
      {
        label: 'Avg. Premium',
        value: premium,
        icon: React.createElement(Percent, { className: 'h-4 w-4' }),
        hint: undefined,
      },
    ]
  },
  ...
  disputes(data: any[]) {
    ...
    const sla =
      (data?.length
        ? data.reduce((s: number, x: any) => s + (x.slaHours || 0), 0) /
          data.length
        : 0
      ).toFixed(1) + 'h'
    const escRate =
      (
        ((data?.filter((x: any) => x.escalated).length ?? 0) * 100) /
        (data?.length || 1)
      ).toFixed(0) + '%'
    return [
      {
  ...
  commissions(rules: any[]) {
    ...
    return [
      ...
      {
        label: 'Last Updated',
        value: rules?.[0]?.updatedAt || '—',
        icon: React.createElement(History, { className: 'h-4 w-4' }),
      },
      {
        label: 'Scopes',
        value: rules?.[0]?.scopes || 0,
  ...
  invoices(inv: any[], webhooks: any[]) {
    ...
    return [
      ...
      },
      {
        ...
        value: failures,
        icon: React.createElement(Webhook, { className: 'h-4 w-4' }),
        hint: undefined,
      },
    ]
  },
  reconciliation(webhooks: any[]) {
    ...
    return [
      ...
      },
      {
        label: 'Providers',
        value: providers,
        icon: React.createElement(Plug2, { className: 'h-4 w-4' }),
        hint: undefined,
      },
      {
        label: 'Open Incidents',
        value: webhooks?.filter((x: any) => x.incidentOpen).length ?? 0,
  ...
  masters(m: any) {
    const cat = m?.categories?.length ?? 0,
      sub = m?.subcategories?.length ?? 0,
      brands = m?.brands?.length ?? 0,
      units = m?.units?.length ?? 0
    return [
      {
        label: 'Categories',
        value: cat,
    ...
    return [
      {
        ...
        value: m?.restrictedStates ?? 0,
        icon: React.createElement(MapPin, { className: 'h-4 w-4' }),
        hint: undefined,
      },
      {
        label: 'Regulated Materials',
        value: m?.regulatedMaterials ?? 0,
        icon: React.createElement(ShieldAlert, { className: 'h-4 w-4' }),
        hint: undefined,
      },
      {
        label: 'Pending Proofs',
        value: m?.pendingProofs ?? 0,
        icon: React.createElement(FileCheck, { className: 'h-4 w-4' }),
        hint: undefined,
      },
      {
        label: 'Authorizations',
        value: m?.authorizations ?? 0,
        icon: React.createElement(ShieldCheck, { className: 'h-4 w-4' }),
        hint: undefined,
      },
    ]
  ...
  cms(pages: any[]) {
    ...
    const published =
      pages?.filter((x: any) => x.status === 'published').length ?? 0
    return [
      {
        label: 'Draft Pages',
        value: drafts,
      ...
      {
        label: 'Last Updated',
        value: pages?.[0]?.updatedAt || '—',
        icon: React.createElement(History, { className: 'h-4 w-4' }),
      },
      {
        label: 'Banners',
        value: pages?.[0]?.banners ?? 0,
  ...
  news(items: any[]) {
    const pending =
      items?.filter((x: any) => x.status === 'pending').length ?? 0
    const published =
      items?.filter((x: any) => x.status === 'published').length ?? 0
    return [
      {
        label: 'Pending',
        value: pending,
      ...
      {
        label: 'Pinned',
        value: items?.filter((x: any) => x.pinned).length ?? 0,
        icon: React.createElement(Pin, { className: 'h-4 w-4' }),
        hint: undefined,
      },
      {
        label: 'Sources',
        value: new Set((items || []).map((x: any) => x.source)).size,
        icon: React.createElement(Plug2, { className: 'h-4 w-4' }),
        hint: undefined,
      },
    ]
  ...
  audit(items: any[]) {
    return [
      {
        label: 'Actions Today',
        value: items?.filter((x: any) => x.today).length ?? 0,
        icon: React.createElement(History, { className: 'h-4 w-4' }),
        hint: undefined,
      },
      {
        label: 'Actors (24h)',
        value: new Set((items || []).map((x: any) => x.actor)).size,
        icon: React.createElement(Users, { className: 'h-4 w-4' }),
        hint: undefined,
      },
      {
        label: 'Entries (7d)',
        value: items?.filter((x: any) => x.week).length ?? 0,
        icon: React.createElement(ClipboardList, { className: 'h-4 w-4' }),
        hint: undefined,
      },
      {
        label: 'Retention',
        value: (items?.[0]?.retentionDays ?? 90) + 'd',
        icon: React.createElement(Shield, { className: 'h-4 w-4' }),
        hint: undefined,
      },
    ]
  ...
  integrations(items: any[]) {
    ...
    return [
      ...
      {
        label: 'Webhooks',
        value: items?.filter((x: any) => x.type === 'webhook').length ?? 0,
        icon: React.createElement(Webhook, { className: 'h-4 w-4' }),
        hint: undefined,
      },
    ]
  ...
  roles(users: any[], roles: any[]) {
    const admins =
      users?.filter((x: any) => x.role === 'Both' || x.role === 'Admin')
        .length ?? 0
    return [
      {
        label: 'Roles',
      ...
      {
        label: 'Last Change',
        value: roles?.[0]?.updatedAt || '—',
        icon: React.createElement(History, { className: 'h-4 w-4' }),
        hint: undefined,
      },
      {
        label: 'Permissions',
        value: roles?.[0]?.perms ?? 0,
        icon: React.createElement(FileCheck, { className: 'h-4 w-4' }),
        hint: undefined,
      },
    ]
  ...
  kyc(data: any[]) {
    ...
    const approved =
      data?.filter((x: any) => x.status === 'approved').length ?? 0
    const rejected =
      data?.filter((x: any) => x.status === 'rejected').length ?? 0
    return [
      {
